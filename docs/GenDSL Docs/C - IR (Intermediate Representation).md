...
| `effort` | `effort "high"` | `string` (only from DSL via compile-time validation; runner accepts `"low"` / `"medium"` / `"high"` / `"max"`). Sent as `body.effort` + `body.thinking: { type: "adaptive" }` to Anthropic models that dropped sampling params (Opus 4.7+, Fable 5, Mythos 5); on those models `max_tokens` is also aliased to `max_output_tokens` on the wire (see `N - Model Identifiers`). Emitted as `null` (present, not omitted — same as `mode`) when the gen didn't declare effort. |
