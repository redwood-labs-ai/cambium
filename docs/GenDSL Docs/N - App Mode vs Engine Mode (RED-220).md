...
7. **Engine-mode authoring polish.** *(Landed 2026-04-20, same day as item 6; see git log.)*
   ...
   - Resolved:
     ...
     - `cli/generate.mjs` — `generateSchema` in engine mode now appends exports directly to `<engineDir>/schemas.ts` (creating the file with a typebox import when absent; idempotent on duplicate name). App-mode behavior unchanged — `src/contracts.ts` is a shared workspace artifact that a scaffolder shouldn't quietly edit.
