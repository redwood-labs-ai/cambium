...
- **`memory`** *(RED-215 phases 2+3 — declare + execute)*: declare per-gen memory slots (`memory :conversation, strategy: :sliding_window, size: 20`). Scopes are `:session`, `:global`, or a named pool defined under `app/memory_pools/<name>.pool.rb`. The pool is authoritative on strategy/embed/keyed_by; the gen can only add reader knobs like `top_k`/`size`. At run time the runner opens one SQLite file per bucket (`runs/memory/<scope>/<key>/<name>.sqlite`), reads the last N entries for `:sliding_window` into a `## Memory` block appended to the system prompt, and — after a successful run with no `write_memory_via` declared — appends one `{input, output}` entry. Session id comes from `CAMBIUM_SESSION_ID` or is auto-generated and echoed to stderr. `--memory-key name=value` supplies values for `keyed_by` slots. `write_memory_via :agent` + `reads_trace_of :primary_agent` + `mode :retro` parse but are not yet wired (phase 4). `:semantic` strategy errors at plan time (phase 5). See [`P - Memory (design note)`](docs/GenDSL%20Docs/P%20-%20Memory%20%28design%20note%29.md).
