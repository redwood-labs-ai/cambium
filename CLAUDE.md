...
- Providers supported for agentic mode:
  ...
  - **Anthropic** *(RED-321)*: requires `ANTHROPIC_API_KEY` or `CAMBIUM_ANTHROPIC_API_KEY`; optional `CAMBIUM_ANTHROPIC_BASEURL` (default `https://api.anthropic.com`). Model id: `"anthropic:<name>"` (e.g. `"anthropic:claude-sonnet-4-6"`, `"anthropic:claude-opus-4-7"`, `"anthropic:claude-haiku-4-5-20251001"`). Prompt caching is on by default — `cache_control: ephemeral` is applied to the system block and the last tool, so a stable system prompt + tool set pays cache creation once and hits on subsequent turns/runs. Cache stats surface as `usage.cache_creation_input_tokens` / `usage.cache_read_input_tokens` in the trace.
