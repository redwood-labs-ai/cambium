...
- **`corrects`**: deterministic post-validation transforms (built-ins: math, dates, currency, citations). App-level `app/correctors/<name>.corrector.ts` files are auto-discovered in app-mode and override built-ins by name (RED-275). Correctors returning `corrected: false` with `severity: 'error'` issues trigger a repair attempt; after the repair passes schema re-validation, the same corrector re-runs to verify the concern was healed (RED-298 `CorrectAfterRepair`). Each declaration accepts a `max_attempts:` kwarg (default 1, ceiling 3): `corrects :regex_x, max_attempts: 3`. When the loop exhausts with errors still pending, the runtime emits `CorrectAcceptedWithErrors` so downstream consumers can refuse output on unhealed errors.
- **`log`** *(RED-282 / RED-302 — trace-fan-out to external observability platforms)*: declare per-gen destinations that receive framework-owned dot-notation events (`<gen>.<method>.<event>`). Profile form `log :app_default` resolves `app/log_profiles/<name>.log_profile.rb`; inline form `log :datadog, include: [:signals], granularity: :run`. Framework built-ins: `:stdout`, `:http_json`, `:datadog`. App plugins under `app/logs/<name>.log.ts` auto-discover. Event vocabulary (run-level): `complete`, `complete_with_warnings`, `failed` (with `reason` field). Sink errors never fail the run — `LogFailed` trace steps instead. See [`P - log`](docs/GenDSL%20Docs/P%20-%20log.md).
- **`cron`** *(RED-273 / RED-305 — scheduled-fire declaration + runtime semantics)*: declare that a gen runs on a recurring schedule. `cron :daily, at: "9:00"` (named vocab) or `cron "30 14 * * 1-5"` (raw crontab). Framework owns declaration + IR + runtime semantics (memory scope `:schedule`, `trace.fired_by`, `fired_by:schedule` ddtag on log events). Framework does NOT own the lifecycle — no daemon. `cambium schedule compile <workspace> --target <k8s-cronjob|crontab|systemd|github-actions|render-cron>` emits deploy-ready manifests invoking `cambium run ... --fired-by schedule:<id>`. Dev: `cambium schedule preview <gen>`, `cambium schedule list`. See [`P - cron (schedule)`](docs/GenDSL%20Docs/P%20-%20cron%20%28schedule%29.md).
...
cambium schedule preview <gen.cmb.rb> [--count N]          # next N fires per schedule (RED-305)
cambium schedule list [<gens-dir>]                         # workspace-wide schedule listing
cambium schedule compile <gens-dir> --target <k8s-cronjob|crontab|systemd|github-actions|render-cron> [--out-dir <dir>] [--image <img>] [--force]   # emit deploy manifests
...
packages/cambium/
  app/
    ...
    logs/           # App log plugins (.log.ts) — custom observability backends (RED-302)
...
- Providers supported for agentic mode:
  ...
  - **Anthropic** *(RED-321, RED-323)*: requires `ANTHROPIC_API_KEY` or `CAMBIUM_ANTHROPIC_API_KEY`; optional `CAMBIUM_ANTHROPIC_BASEURL` (default `https://api.anthropic.com`). Model id: `"anthropic:<name>"` (e.g. `"anthropic:claude-sonnet-4-6"`, `"anthropic:claude-opus-4-7"`, `"anthropic:claude-haiku-4-5-20251001"`). Prompt caching is on by default (system block, last tool, last document). Native document input via typed `ir.context` envelopes (`{ kind: 'base64_pdf' \| 'base64_image', data, media_type }`) — Anthropic only, size-capped, non-Anthropic providers fail fast. See [`N - Model Identifiers`](docs/GenDSL%20Docs/N%20-%20Model%20Identifiers.md) § Native document input for full envelope shape, size caps, grounding interaction, and `CAMBIUM_MAX_DOC_BYTES_PER_RUN`.
...
- **`cambium-security`** — reviews changes that touch tool dispatch, egress, the `security`/`budget` policy surface, tool registration, or code-generation paths. Enforces the invariants in the "Tool / action dispatch + egress", "Exec substrate", and "Code-gen + path-traversal guards" clusters below — locked in by RED-137 (SSRF guard, IP pinning, dispatch-site gates, budget pre-call checks), RED-214 (per-slot mixing, pack-name regex), RED-209 (plugin permission honesty), and RED-222 (scaffolder path-traversal + overwrite protection). **Use proactively** when modifying `packages/cambium-runner/src/tools/**`, `packages/cambium-runner/src/step-handlers.ts`, `packages/cambium-runner/src/runner.ts`, `packages/cambium-runner/src/triggers.ts`, adding a `*.tool.json` / `*.tool.ts` / `*.action.json` / `*.action.ts` / `*.policy.rb`, or changing the Ruby DSL's `security`/`budget` shape.
- **`cambium-docs`** — reviews the same-PR alignment between code and docs. Catches stale wikilinks after a rename, new DSL methods in `runtime.rb` that aren't documented, new IR fields missing from `C - IR`, new trace step types missing from `C - Trace`, and README project-structure tree drifting behind disk layout. **Use proactively** when modifying `ruby/cambium/runtime.rb`, `ruby/cambium/compile.rb`, `packages/cambium-runner/src/runner.ts` / `packages/cambium-runner/src/step-handlers.ts` / `packages/cambium-runner/src/triggers.ts` (new trace types), adding/renaming files under `docs/GenDSL Docs/`, touching `CLAUDE.md` / `README.md`, or modifying the editor-assist surface (`cli/generate.mjs`, `cli/lint.mjs`, `cli/init.mjs`, `vscode/cambium-syntax/**`) — parity between those surfaces and the DSL matters. Calibrated to flag user-visible misalignments, NOT prose or style nits.

#### When to invoke (and when to skip)

These agents have a real context cost. This session pattern worked well: invoke them when a change meaningfully interacts with their territory; skip them when it doesn't.

**Run `cambium-security` when the change:**
- touches tool or action dispatch (`handleToolCall` / `dispatchAction` / registration)
- changes the `security` / `budget` DSL surface or compile normalization
- changes the exec substrate, network allowlist, or filesystem allowlist plumbing
- adds a new named-symbol surface that interpolates into `File.join` / `path.join` (new pack-like file loader, new scope key, new scaffolder name)
- adds or modifies a scaffolder that writes executable files (`.ts` / `.rb` / tool or action handlers)
- changes `parseBudget`, `trackBudgetFromTraceStep`, or any new `Repair` trace-step site

**Run `cambium-docs` when the change:**
- adds a DSL primitive, new IR field, or new trace step type
- renames or adds a file under `docs/GenDSL Docs/`
- modifies `CLAUDE.md` "Key concepts" or the project-structure tree
- modifies `README.md` project-structure tree or "Key features"
- adds a new `app/<type>/` convention directory (which every meta surface — scaffolders, lint, LSP — needs to track)
- adds a new DSL primitive — because the meta surfaces (`cli/generate.mjs` GENERATORS, `vscode/cambium-syntax/src/server.js` `PRIMITIVE_DOCS` and grammar, snippets) all need parity

**Skip both when the change is:**
- a one-line fix with no new invariant surface
- pure test additions (existing behavior, new coverage)
- editor-assist-only edits that don't imply DSL doc drift (a new snippet, a completion tweak)
- refactors that preserve behavior and don't introduce new symbols / paths / dispatch sites

**When in doubt, run them — the agents are calibrated to stay quiet when nothing flags.** A clean "all invariants hold, 0 drifts found" comes back in a few seconds and is cheaper than missing a drift.

### Non-obvious invariants

Things that will bite you if you don't know them. Organized by cluster — when a change touches one area, the bullets in that cluster are the ones most likely to be relevant.
...
The pattern: anywhere a user-picked symbol or string gets interpolated into a filesystem path, a regex guard runs BEFORE `File.join` / `path.join` — because `node:path` and Ruby's `File.join` normalise `..` silently. Every one of these guards exists because the bare join was unsafe.

- **Pack file names are restricted to `/\A[a-z][a-z0-9_]*\z/` (RED-214).** A symbol like `:"../foo"` would otherwise interpolate into `File.join` and escape `app/policies/`. The check lives in `PolicyPack.load`. Don't relax it.
...
- **`Genfile.toml [types].contracts` paths are validated at CLI startup, not silently skipped (RED-274).** `resolveGenfileContracts` in `packages/cambium-runner/src/genfile.ts` rejects absolute paths, null bytes, and any relative path whose `resolve()` escapes the Genfile directory (checked via `relative(genfileDir, abs).startsWith('..')`). Each resolved path is then `await import()`ed via `pathToFileURL`. This mirrors the RED-214 pack-name regex and RED-215 pool-name regex stance. If you add a new `Genfile.toml` section that loads user-declared file paths (e.g., `[tools].handlers` or `[policies].packs`), copy the same guards (null byte + `isAbsolute` + `relative(dir, abs).startsWith('..')`) — bare `path.join` normalises `..` silently and is not safe. Picker vs validator: `findGenfileDir(ir.entry.source)` (same file) chooses *which* `Genfile.toml` gets loaded by walking up from the gen file (mirrors `resolveEngineDir`'s shape — fixed filename, no user-controlled segments interpolated); `resolveGenfileContracts` then validates the `[types].contracts` entries inside it. The walk-up was added because the older cwd-only lookup found the wrong workspace's contracts when a Cambium app ran with cwd inside a different Cambium workspace.
- **Engine detection has a cwd-fallback for cross-env IRs (RED-353).** When `ir.entry.source` is an absolute path that doesn't exist at run time (the canonical case: IR compiled on the host, run in a container with a different filesystem layout — the path was `/Users/Steve/...` baked in by automated tooling), `resolveEngineDir(entry.source)` returns null. Without a fallback, the runner silently falls through to app mode and tries to load engine schemas from a Genfile that doesn't have them. `findEngineDirFromCwd(cwd)` (in `packages/cambium-runner/src/engine-root.ts`) is the fallback: walk up from cwd looking for the same `cambium.engine.json` sentinel. `runGenFromIr` only invokes it when the source-based detection failed AND `entry.source` is unreachable on disk — source-anchored detection still wins when the path exists, so the explicit "engine discovery follows source not cwd" property of `engine_mode_e2e` (run-from-anywhere) is preserved. Operator contract: at run time, cwd is the engine dir or an ancestor of it. The fallback also passes the resolved `engineDir` explicitly to `runGen` so the inner re-detection doesn't undo the cwd resolution.
- **App correctors: basename regex + export name must match + realpath escape guard (RED-275).** `loadAppCorrectors` in `packages/cambium-runner/src/correctors/app-loader.ts` scans `<genfileDir>/app/correctors/*.corrector.ts`, requires each basename match `/^[a-z][a-z0-9_]*$/` (same stance as RED-214/215), `realpath`s the file and rejects any target that escapes the correctors dir (`relative(realDir, realFile).startsWith('..')`), and requires the module to export a function binding matching the basename. A throwing corrector is caught in `runCorrectorPipeline` and converted to a `severity: 'error'` issue so user code can't crash the run. The mutable module-global registry has been replaced by per-`runGen` isolation — see the RED-299 bullet below.
...
#### Pipeline structure + compile-time enforcement
...
- **Corrector error-severity issues feed the repair loop (RED-275, extended RED-298).** `runner.ts` now runs a per-corrector loop with per-corrector `max_attempts` (default 1, ceiling 3 enforced at compile time). Each iteration: Repair → `ValidateAfterCorrectorRepair` → **re-run the corrector** (`CorrectAfterRepair`). If `max_attempts` exhausts with errors still pending, the runtime emits `CorrectAcceptedWithErrors` (`ok: false`) as a greppable terminal state — the run does NOT fail (output is schema-valid) but downstream consumers can refuse. IR shape: `policies.correctors` is `Array<{name, max_attempts}>`; pre-RED-298 bare-string arrays still run via a back-compat normalizer that fills `max_attempts: 1`. The TS normalizer ALSO clamps `max_attempts` to `[1, 3]` — same belt-and-suspenders stance as RED-239 (Ruby enforces, TS re-enforces) so hand-written IRs can't bypass. Every Repair in the loop still goes through `pushRepairStep` so budget tracking is uniform.
- **Correctors are per-`runGen`, not process-global (RED-299).** The corrector registry is built fresh inside each `runGen` call from `{ ...builtinCorrectors, ..._getLegacyAppCorrectors(), ...opts.correctors, ...engineSiblings }` in that precedence order. Long-lived engine-mode hosts that drive multiple gens in one process get clean isolation: App A's correctors cannot leak into App B's run. The old module-global `correctors` export was removed; `registerAppCorrectors` remains as a deprecated back-compat shim that emits a one-time stderr warning and writes into a legacy fallback map the runner merges at low precedence. New engine-mode code MUST use `RunGenOptions.correctors`. `handleCorrect` and `runCorrectorPipeline` both take the correctors map as an explicit parameter — never read from a module-global. A future refactor that reintroduces a global read re-opens the silent cross-app override bug RED-281 caught.
- **Corrector re-run after schema-repair is a correctness invariant (RED-298).** Every `ValidateAfterCorrectorRepair` that passes schema validation MUST be followed by a `CorrectAfterRepair` re-run of the same corrector (gated only by "repair produced parseable output"). Removing or skipping that re-run re-introduces the pre-RED-298 bug: the runner would report "healed" based only on schema shape, not on the corrector's actual concern. Symptom that bit us (curator JWT dogfood, 2026-04-20): repaired regex still failed its own test cases, `ValidateAfterCorrectorRepair ok:true` made the run appear successful, downstream belt-and-suspenders caught it. Also: `CorrectAcceptedWithErrors` is gated on `!correctorSchemaBroke` — a mutating corrector that breaks schema on re-run must not also emit "accepted with unhealed errors" for the pre-break error set.
...
- **Log event vocabulary is framework-owned, not user-extensible (RED-282 / RED-302).** Event names follow `<snake_gen>.<method>.<event>` with a fixed run-level vocabulary (`complete`, `complete_with_warnings`, `failed`) and step-level events that mirror trace step types. Users cannot invent arbitrary event names — the Rails-y opinionated stance is "framework owns the taxonomy so dashboards remain tractable across shops." A new event type requires extending the runner AND the `C - Trace (observability).md` row (same invariant as any new trace step type). If a gen author needs a bespoke event, emit it from a tool via `granularity: :step` — don't widen the enum.
- **Log emission is fire-and-forget, never fails the run (RED-282 / RED-302).** Every log-sink dispatch in `emitLogEvent` is wrapped; backend errors produce a `LogFailed` trace step and continue. A Datadog outage or missing API key must not stop a gen from returning its validated output. If you add a new emission site in `runner.ts`, route through `emitLogEvent` — never call a sink directly or wrap it with logic that re-throws. The async emission is awaited at the three run-outcome sites (success, complete_with_warnings, budget-exceeded failure) so the trace reflects the outcome before `runGen` returns.
- **Log profile references are resolved at compile time, inlined into the IR (RED-282 / RED-302).** `log :app_default` in a gen is rewritten by the Ruby compiler into an array of `{destination, include, granularity, endpoint?, api_key_env?, _profile}` entries — the TS runner never loads profile files itself. Matches the RED-214 policy-pack stance. `_profile` metadata is preserved for trace observability but runner doesn't branch on it. If a profile file is missing and the name isn't a framework built-in, it falls through to the inline form (destination name passed to runtime registry); the runtime emits `LogFailed` if the name doesn't resolve.
- **Cambium owns cron semantics, NOT the lifecycle (RED-273 / RED-305).** A `cron :daily` declaration compiles to `policies.schedules[]` with a stable id (`<snake_gen>.<method>.<slug>`). The runner never watches a clock, never runs a daemon. Instead, operators compile to their platform's scheduler (`cambium schedule compile --target <...>`) and that scheduler invokes `cambium run ... --fired-by schedule:<id>`. The `--fired-by` flag is validated against `ir.policies.schedules[]` at startup — unknown ids are a hard error so typos in cron manifests fail fast. Don't introduce any code path that invokes a gen on a schedule from within the Cambium process; the architecture assumes external scheduling. If a real forcing case emerges for an in-process scheduler, add it as a new `--schedule-target=cambium-serve` compile target; do not short-circuit the external-scheduler stance.
- **`memory scope: :schedule` requires both compile-time AND runtime guards (RED-305).** Compile-time: memory with `scope: :schedule` MUST be paired with at least one `cron` declaration on the same gen — otherwise the scope has no identity (bucket key). Runtime: the run MUST carry `--fired-by schedule:<id>`; `resolveBucketPath` throws `requires a scheduled fire` otherwise. Both guards exist to prevent silent misuse where interactive runs would write to an unkeyed bucket. If you add a new memory scope value, apply both guards symmetrically.
