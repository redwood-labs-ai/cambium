...
- **`memory`** *(RED-215 phase 2 — compile-time only for now)*: declare per-gen memory slots (`memory :conversation, strategy: :sliding_window, size: 20`). Scopes are `:session`, `:global`, or a named pool defined under `app/memory_pools/<name>.pool.rb`. The pool is authoritative on strategy/embed/keyed_by; the gen can only add reader knobs like `top_k`/`size`. `write_memory_via :agent` + `reads_trace_of :primary_agent` + `mode :retro` wire up a memory-writer gen. Runtime execution (sqlite-vec backend + trace events) lands in phase 3. See [`P - Memory (design note)`](docs/GenDSL%20Docs/P%20-%20Memory%20%28design%20note%29.md).
