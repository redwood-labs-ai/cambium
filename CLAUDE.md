...
- **Corrector error-severity issues feed the repair loop (RED-275).** `runner.ts` now fires one additional repair attempt when `handleCorrect` returns `meta.issues` containing any `severity: 'error'` entry — mirrors the existing grounding path. A new `ValidateAfterCorrectorRepair` trace step is emitted on the re-validation. Don't push a new `Repair` trace step in any corrector-adjacent path without pairing it with `budgetTrack(repair.result)` — without the track, the tokens spent on the feedback repair are invisible to the budget gate.
